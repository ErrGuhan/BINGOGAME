...
export function useBingoGame(initialRoomCode?: string) {
  ...
  const winner = game?.winner_id ? (game.winner_id === p1?.id ? p1 : p2) : null;
  const isWinner = Boolean(player && winner && player.id === winner.id);
  ...
  const handleRealtimeEvent = useCallback((event: string, payload: unknown) => {
    ...
    if (event === 'NUMBER_CALLED') {
      ...
      const d = data as { allReady: boolean; status: string; currentTurnPlayerId: string };
      if (d.allReady) {
        setGame(prev => prev ? {
          ...prev,
          status: 'playing',
  ...
  const setBoard = async (board: number[]) => {
    ...
    try {
      const supabase = getSupabase();
      if (supabase && isSupabaseConfigured()) {
        const { error } = await supabase.rpc('set_player_board', {
          p_game_id: game.id,
          p_session_id: sessionId,
          p_board: board,
        });
